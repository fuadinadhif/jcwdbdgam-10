...
const pointsMoreThan100 = points.filter((element) => element > 100);
console.log(pointsMoreThan100);

/* -------------------------------- filter 2 -------------------------------- */
// 1. me-loop array yang difilter
// 2. di setiap loop-nya dia akan menjalankan argument-nya/callback-nya
...
const nameStartWithAOrK = studentsName.filter(
  (value) =>
    value.toUpperCase().startsWith("A") || value.toUpperCase().startsWith("K")
);
console.log(nameStartWithAOrK);

/* ----------------------------------- map ---------------------------------- */
const raports = [85, 90, 100];
const raportsWithBonus = raports.map((value) => {
